(function (global, factory) {
    ...
    class Viewer {
        ...
        constructor(rootElement = null, cameraUp = [0, 1, 0], initialCameraPos = [0, 10, 15], initialCameraLookAt = [0, 0, 0],
            ...
            this.cameraUp = new THREE__namespace.Vector3().fromArray(cameraUp);
            this.initialCameraPos = new THREE__namespace.Vector3().fromArray(initialCameraPos);
            this.initialCameraLookAt = new THREE__namespace.Vector3().fromArray(initialCameraLookAt);
        ...
        updateSplatRenderTargetForRenderDimensions(renderDimensions) {
            this.splatRenderTarget = new THREE__namespace.WebGLRenderTarget(renderDimensions.x, renderDimensions.y, {
                format: THREE__namespace.RGBAFormat,
                stencilBuffer: false,
                depthBuffer: true,

            });
            this.splatRenderTarget.depthTexture = new THREE__namespace.DepthTexture(renderDimensions.x, renderDimensions.y);
        ...
        updateSplatMeshAttributes(colors, centerCovariances, vertexCount) {
            ...
            const centerCovarianceTexture = new THREE__namespace.DataTexture(paddedCenterCovariances, CENTER_COVARIANCE_DATA_TEXTURE_WIDTH,
                                                                  CENTER_COVARIANCE_DATA_TEXTURE_HEIGHT, THREE__namespace.RGBAFormat, THREE__namespace.FloatType);
            ...
            const colorTexture = new THREE__namespace.DataTexture(paddedColors, COLOR_DATA_TEXTURE_WIDTH,
                                                       COLOR_DATA_TEXTURE_HEIGHT, THREE__namespace.RGBAFormat, THREE__namespace.FloatType);
        ...
        updateSplatMeshUniforms = function() {
            ...
            return function() {
                ...
                if (vertexCount > 0) {
                    ...
                    this.cameraFocalLength = (renderDimensions.y / 2.0) / Math.tan(THREE_CAMERA_FOV / 2.0 * THREE__namespace.MathUtils.DEG2RAD);
        ...
        createDebugMeshes(renderOrder) {
            ...
            let sphereMesh = new THREE__namespace.Mesh(sphereGeometry, this.buildDebugMaterial(0xff0000));
            sphereMesh.renderOrder = renderOrder;
            debugMeshRoot.add(sphereMesh);
            sphereMesh.position.set(-50, 0, 0);

            sphereMesh = new THREE__namespace.Mesh(sphereGeometry, this.buildDebugMaterial(0xff0000));
            sphereMesh.renderOrder = renderOrder;
            debugMeshRoot.add(sphereMesh);
            sphereMesh.position.set(50, 0, 0);

            sphereMesh = new THREE__namespace.Mesh(sphereGeometry, this.buildDebugMaterial(0x00ff00));
            sphereMesh.renderOrder = renderOrder;
            debugMeshRoot.add(sphereMesh);
            ...
            sphereMesh = new THREE__namespace.Mesh(sphereGeometry, this.buildDebugMaterial(0xffaa00));
            sphereMesh.renderOrder = renderOrder;
            debugMeshRoot.add(sphereMesh);
        ...
        createSecondaryDebugMeshes(renderOrder) {
            ...
            let boxMesh = new THREE__namespace.Mesh(boxGeometry, this.buildDebugMaterial(boxColor));
            boxMesh.renderOrder = renderOrder;
            debugMeshRoot.add(boxMesh);
            boxMesh.position.set(-separation, 0, -separation);

            boxMesh = new THREE__namespace.Mesh(boxGeometry, this.buildDebugMaterial(boxColor));
            boxMesh.renderOrder = renderOrder;
            debugMeshRoot.add(boxMesh);
            boxMesh.position.set(-separation, 0, separation);
        ...
        updateView = function() {
            ...
            const tempVector2 = new THREE__namespace.Vector2();
        ...
        buildMaterial() {
            ...
            const uniforms = {
                ...
                'centerCovarianceTextureSize': {
                    'type': 'v2',
                    'value': new THREE__namespace.Vector2(CENTER_COVARIANCE_DATA_TEXTURE_WIDTH, CENTER_COVARIANCE_DATA_TEXTURE_HEIGHT)
                },
                'colorTextureSize': {
                    'type': 'v2',
                    'value': new THREE__namespace.Vector2(COLOR_DATA_TEXTURE_WIDTH, COLOR_DATA_TEXTURE_HEIGHT)
                }
            };
            ...
            const material = new THREE__namespace.ShaderMaterial({
                ...
                blendSrc: THREE__namespace.OneMinusDstAlphaFactor,
                blendDst: THREE__namespace.OneFactor,
                blendSrcAlpha: THREE__namespace.OneMinusDstAlphaFactor,
                blendDstAlpha: THREE__namespace.OneFactor,
                depthTest: true,
                depthWrite: false,
        ...
        buildGeomtery(splatBuffer) {
            ...
            const splatColors = new THREE__namespace.InstancedBufferAttribute(splatColorsArray, 4, false);
            splatColors.setUsage(THREE__namespace.DynamicDrawUsage);
            geometry.setAttribute('splatColor', splatColors);

            const splatCentersArray = new Float32Array(vertexCount * 9);
            const splatCenters = new THREE__namespace.InstancedBufferAttribute(splatCentersArray, 9, false);
            splatCenters.setUsage(THREE__namespace.DynamicDrawUsage);
...
//# sourceMappingURL=gaussian-splat-3d.umd.cjs.map
